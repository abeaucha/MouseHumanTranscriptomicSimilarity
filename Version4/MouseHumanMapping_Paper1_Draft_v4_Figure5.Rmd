...
# heatmapPalette <- colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(100)
# heatmapPalette <- colorRampPalette(brewer.pal(n = 9, name = "PuRd"))(100)
heatmapPalette <- magma(n = 100, direction = 1, begin = 0.35) #Not bad
# heatmapPalette <- rocket(n = 100, direction = 1, begin = 0.2) #Not bad
# heatmapPalette <- colorRampPalette(colors = carto_pal(n = 7, name = "Geyser"))(100) #Not bad
heatmapAnnotationColours_CxType <- c("#00325C", "#B27700")
# heatmapAnnotationColours_CxType <- carto_pal(n = 11, name = "Safe")[c(1,8)]
...
fig5_wcss <- ggplot(dfWCSS, aes(x = k, y = wcss, col = data)) + 
  ...
  scale_color_discrete(labels = c("Homologous genes", "Average latent space"), name = NULL) + 
...
```{r}
grid.newpage()
grid.draw(getGrob(fig5_wcss_grob, "guide-box.11-5-11-5"))
```


```{r}
grid.ls(fig5_wcss_grob)
```
...
```{r fig5_grob, fig.width=8.5, fig.height=7.5}
...
#                                fig5_heatmap_rowtitle_grob, #6
#                                fig5_heatmap_colnames_grob, #7
#                                fig5_heatmap_annotationlegend_grob, #8
#                                fig5_heatmap_legend_grob, #9
#                                fig5_heatmap_coltitle_grob, #10
#                                fig5_regionaldist_ytitle_grob, #11
#                                fig5_regionaldist_yaxis_grob, #12
#                                fig5_regionaldist_panel_grob, #13
#                                fig5_regionaldist_xaxis_grob, #14
#                                fig5_regionaldist_xtitle_grob, #15
#                                fig5_cortexdist_ytitle_grob, #16
#                                fig5_cortexdist_yaxis_grob, #17
#                                fig5_cortexdist_panel_grob, #18
#                                fig5_cortexdist_xaxis_grob, #19
#                                fig5_cortexdist_xtitle_grob, #20
#                                layout_matrix = rbind(c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA),
#                                                      c(NA, NA, NA,  1,  1,  1,  1, NA, NA, NA, NA),
#                                                      c( 2,  2,  3,  4,  4,  4,  4,  5,  5,  5,  6),
#                                                      c(NA, NA, NA,  7,  7,  7,  7,  8,  8,  8, NA),
#                                                      c(NA, NA, NA,  7,  7,  7,  7,  9,  9,  9, NA),
#                                                      c(NA, NA, NA, 10, 10, 10, 10,  9,  9,  9, NA),
#                                                      c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA),
#                                                      c(11, 12, 12, 13, 16, 17, 18, 18, 18, 18, NA),
#                                                      c(NA, NA, NA, 14, NA, NA, 19, 19, 19, 19, NA),
#                                                      c(NA, NA, NA, 15, NA, NA, 20, 20, 20, 20, NA),
#                                                      c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA)),
#                                widths = unit(c(0.5, 1, 0.3, 3, 0.5, 0.25, 0.85, 0.25, 0.5, 0.75, 0.25), 'inch'),
#                                heights = unit(c(0.25, 1, 2, 0.6, 0.2, 0.5, 0.4, 2, 0.25, 0.25, 0.25), 'inch'))
fig5_plots_grob <- arrangeGrob(fig5_heatmap_coltree_grob, #1
                               fig5_heatmap_rowtree_grob, #2
                               fig5_heatmap_rowannotations_grob, #3
                               fig5_heatmap_matrix_grob, #4
                               gTree(children = gList(rectGrob(),fig5_heatmap_rownames_grob)), #5
                               # gTree(children = gList(rectGrob(), fig5_heatmap_rowtitle_grob)), #6
                               fig5_heatmap_colnames_grob, #6
                               fig5_heatmap_annotationlegend_grob, #7
                               fig5_heatmap_legend_grob, #8
                               fig5_heatmap_coltitle_grob, #9
                               fig5_wcss_panel_grob, #10
                               fig5_wcss_ytitle_grob, #11
                               fig5_wcss_yaxis_grob, #12,
                               gTree(children = gList(rectGrob(),fig5_wcss_xaxis_grob)), #13
                               gTree(children = gList(rectGrob(),fig5_wcss_xtitle_grob)), #14
                               fig5_wcss_legend_grob, #15
                               ...
                               gTree(children = gList(rectGrob(), fig5_cortexdist_ytitle_grob)), #20
                               gTree(children = gList(rectGrob(),fig5_cortexdist_yaxis_grob)), #21
                               fig5_cortexdist_panel_grob, #22
                               fig5_cortexdist_xaxis_grob, #23
                               fig5_cortexdist_xtitle_grob, #24
                               layout_matrix = rbind(c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA),
                                                     c(NA, NA, NA,  1,  1, NA, NA, NA, NA, NA, NA, NA, NA),
                                                     c( 2,  2,  3,  4,  4,  5,  5,  5,  5, 11, 12, 10, NA),
                                                     c(NA, NA, NA,  6,  6,  7,  7,  7,  7, NA, NA, 13, NA),
                                                     c(NA, NA, NA,  6,  6,  7,  7,  7,  7, NA, NA, 14, NA),
                                                     c(NA, NA, NA,  6,  6,  7,  7,  7,  7, NA, NA, 15, NA),
                                                     c(NA, NA, NA,  6,  6,  8,  8,  8,  8, NA, NA, 15, NA),
                                                     c(NA, NA, NA,  9,  9,  8,  8,  8,  8, NA, NA, 15, NA),
                                                     c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA),
                                                     c(16, 17, 17, 17, 18, NA, 21, 22, 23, 23, 23, 23, NA),
                                                     c(NA, NA, NA, NA, 19, NA, NA, NA, 24, 24, 24, 24, NA),
                                                     c(NA, NA, NA, NA, 20, NA, NA, NA, 25, 25, 25, 25, NA),
                                                     c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA)),
                               widths = unit(c(0.25, 0.2, 0.2, 0.9, 2.75, 0.5, 0.25, 0.35, 0.3, 0.35, 0.3, 1.7, 0.1), 'inch'),
                               heights = unit(c(0.25, 1, 2, 0.2, 0.2, 0.2, 0.2, 0.5, 0.4, 2, 0.25, 0.25, 0.25), 'inch'))

#Grob for panel labels
fig5_panelid_grob <- textGrob(c("A.", "B.", "C."),
                              x = unit(c(0.4, 0.4, 5.2), 'inch'),
                              y = unit(c(7, 2.9, 2.9), 'inch'),
