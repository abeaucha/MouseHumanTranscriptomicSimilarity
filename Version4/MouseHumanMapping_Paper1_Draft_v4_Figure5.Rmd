...
```{r fig.width=10}
psankey_meansim <- ggplot(temp4,
       aes(axis1 = fct_rev(Seed), axis2 = Target, y = y)) + 
  geom_alluvium(aes(fill = meansim),
                col = "grey50") + 
  geom_stratum() + 
  geom_text(stat = "stratum",
            aes(label = after_stat(stratum),
...
ggplot(temp5, aes(x = Target, y = Seed)) + 
  geom_tile(aes(fill = Region16),
            col = "grey50",
            alpha = 0.7) + 
  geom_point(aes(size = meansim, col = IsMaxProp),
             na.rm = T) + 
  scale_fill_manual(values = coloursHuman) + 
  scale_size_continuous(range = c(0, 4)) + 
  scale_color_manual(values = c("grey50", "black")) + 
...
temp5 %>% 
  ...
  filter(!is.na(prop)) %>% 
  select(Seed, Target, Proportion = prop, AvgCorrelation = meansim) %>% 
...
```{r}
matSimCx <- dfSimilarityProfiles_Cx %>% 
  filter(Seed %in% structsMouse_Cx,
...
pheatmap(matSimCx, cluster_cols = T, cluster_rows  = F)
```

```{r fig.width = 10, fig.height = 8}
pheatmap(matSimCx, cluster_cols = T, cluster_rows  = T)
```
