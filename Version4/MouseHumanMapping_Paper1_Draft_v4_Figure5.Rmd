...
temp4 <- temp4 %>% 
  ...
  mutate(Region16 = ifelse(is.na(Region16), "Not cortical", Region16),
         ...
         Region16 = factor(Region16, levels = c("limbic lobe", "frontal lobe", "insula", "occipital lobe","parietal lobe", "temporal lobe", "Not cortical")))
...
```{r fig.width = 10}
temp4 <- temp4 %>% mutate(prop = ifelse(prop == 0, NA, prop))
ggplot(temp4, aes(x = Target, y = Seed)) + 
...
ggplot(temp4, aes(x = Target, y = Seed)) + 
  ...
  #           col = "grey50",
  #           alpha = 0.7) + 
  geom_tile(aes(fill = factor(1)),
            col = "grey75") + 
  geom_point(aes(size = prop, col = Region16),
             na.rm = T) +
...
```{r}
ggplot(temp3, aes(x = Target, y = Seed, fill = prop, col = IsMaxProp, size = IsMaxProp)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "black", breaks = seq(0, 1, 0.25)) +
