...
clusteredHeatmap_MLP <- pheatmap(matSim_Cx_MLP_norm, cluster_cols = T, cluster_rows  = T)
```

```{r fig.width = 10, fig.height = 8}
matSim_Cx_AllGenes_norm <- matSim_Cx_AllGenes/max(matSim_Cx_AllGenes)
clusteredHeatmap_AllGenes <- pheatmap(matSim_Cx_AllGenes_norm, cluster_cols = T, cluster_rows  = T)
```
...
dfWCSS_Cols_MLP <- tibble(k = 1:ncol(matSim_Cx_MLP_norm),
                          wcss = computeWCSS(x = matSim_Cx_MLP_norm, clusteredHeatmap_MLP$tree_col),
                          data = "MLP",
                          axis = "Mouse")
dfWCSS_Cols_AllGenes <- tibble(k = 1:ncol(matSim_Cx_AllGenes_norm),
                               wcss = computeWCSS(x = matSim_Cx_AllGenes_norm, clusteredHeatmap_AllGenes$tree_col),
                               data = "AllGenes",
                               axis = "Mouse")

dfWCSS_Rows_MLP <- tibble(k = 1:nrow(matSim_Cx_MLP_norm),
                          wcss = computeWCSS(x = t(matSim_Cx_MLP_norm), clusteredHeatmap_MLP$tree_row),
                          data = "MLP",
                          axis = "Human")
dfWCSS_Rows_AllGenes <- tibble(k = 1:nrow(matSim_Cx_AllGenes_norm),
                               wcss = computeWCSS(x = t(matSim_Cx_AllGenes_norm), clusteredHeatmap_AllGenes$tree_row),
                               data = "AllGenes",
                               axis = "Human")
...
dfClustLabels_MLP <- getClusterLabels(x = matSim_Cx_MLP, hc_tree = clusteredHeatmap_MLP$tree_col) %>% 
