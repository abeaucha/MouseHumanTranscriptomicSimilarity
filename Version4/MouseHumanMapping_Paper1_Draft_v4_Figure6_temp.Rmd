...
dfExampleModel <- dfExampleModel %>% 
  mutate(CaudateResiduals = residuals(mod_example))

mod_example_putamen <- lm(HumanPutamen ~ CaudateResiduals, data = dfExampleModel)
...
pcaudate <- ggplot(dfExampleModel, aes(x = MouseCPVoxel, y = HumanCaudate)) + 
  geom_point() + 
  ...
  coord_cartesian(xlim = c(-5.5, 5.5),
                  ylim = c(-6, 6)) +
  labs(x = "Mouse caudoputamen sample voxel expression",
       y = "Human caudate average expression") + 
  theme_bw()

pputamen <- ggplot(dfExampleModel, aes(x = CaudateResiduals, y = HumanPutamen)) + 
  geom_point() + 
  ...
  coord_cartesian(xlim = c(-5.5, 5.5),
                  ylim = c(-6, 6)) +
  labs(x = "Caudate model residuals",
       y = "Human putamen average expression") + 
  theme_bw()
...
for(i in 1:ncol(matExprMouse_CP)){
  mod_caudate <- lm(matExprHuman_Striatum[,"caudate nucleus"] ~ matExprMouse_CP[,i])
  mod_caudate_resids <- residuals(mod_caudate)
  
  mod_putamen <- lm(matExprHuman_Striatum[,"putamen"] ~ mod_caudate_resids)
  
  dfParams[i, "beta"] <- summary(mod_putamen)$coef["mod_caudate_resids", "Estimate"]
  dfParams[i, "sigma_beta"] <- summary(mod_putamen)$coef["mod_caudate_resids", "Std. Error"]
  dfParams[i, "tval"] <- summary(mod_putamen)$coef["mod_caudate_resids", "t value"]
  dfParams[i, "pval"] <- summary(mod_putamen)$coef["mod_caudate_resids", "Pr(>|t|)"]
}
```

```{r}
voxelSample <- sample(1:ncol(matExprMouse_CP), size = 8, replace = F)
matExprHuman_Striatum[,"caudate nucleus"]
matExprMouse_CP[,voxelSample]
```
...
```{r}
library(ica)
temp <- icajade(X = matExprMouse_scaled,
                nc = 80,
                center = FALSE)
```

```{r}
temp <- prcomp(x = matExprMouse_scaled,
               center = F, scale = F)
```
...
```{r}
matSim_CP_mouse <- buildSimilarityMatrix(x1 = matExprMouse_CP, x2 = matExprMouse_CP)
```

```{r}
hist(matSim_CP_mouse)

for(j in 1:ncol(matSim_CP_mouse)){
  matSim_CP_mouse[j,j] <- NA
}
...
```{r}
voxelSample <- sample(1:ncol(matSim_CP_mouse), size = 100, replace = F)
pheatmap(matSim_CP_mouse[voxelSample, voxelSample],
...
```{r fig.width = 10}
voxelSample <- sample(1:ncol(matExprMouse_CP), size = 100, replace = F)
temp <- dist(t(matExprMouse_CP[,voxelSample]), method = "euclidean")
hclust_temp <- hclust(temp)
plot(hclust_temp)
```

```{r}
clusterLabels <- cutree(hclust_temp, k = 3)

arrayClusters <- emptyArray
names(arrayClusters) <- names(emptyArray)
indVoxelMatch <- match(names(clusterLabels), names(emptyArray))
arrayClusters[indVoxelMatch] <- clusterLabels
attributes(arrayClusters) <- attributes(dsurqeLabels_200um)
```
...
outpaths <- str_c(pathHome, "Draft/", "Version4/", outfiles)
...
fig6_ss_base <- sliceSeries(nrow = 1, ncol = 8, begin = 144, end = 192) %>%
...
fig6_ss_labels_grob <- textGrob(rev(humanTargets_labels),
                                x = 0.75,
                                just = "right",
...
```{r fig6-grob, fig.width = 9, fig.height = 6.25}
#Figure 6 main grob
fig6_plots_grob <- arrangeGrob(fig6_histogram_title_grob, #1
                               ...
                               # rectGrob(),
                               fig6_ss_labels_grob, #5
                               fig6_ss_grob, #6
                               fig6_ss_legend_grob, #7
