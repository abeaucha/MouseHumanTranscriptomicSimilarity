...
# library(viridis)
# library(rcartocolor)
heatmapColours <- colorRampPalette(rev(brewer.pal(n = 11, name = "RdYlBu")))(255)
heatmapBreaks <- seq(-0.4, 0.4, length.out = 255)
# heatmapColours <- colorRampPalette(c(mako(n = 5, direction = 1, begin = 0.35), "white", rocket(n = 5, direction = -1, begin = 0.35)))(100)
# heatmapColours <- colorRampPalette(carto_pal(n = 7, name = "TealRose"))(100)
...
#2. Mouse label lines
#Positions of the lines are dependent on text positionsU
...
arrayInd <- floor(arrayFrac*length(heatmapColours))

ssPalettePositive <- colorRampPalette(heatmapColours[arrayInd[2]:length(heatmapColours)])(255)
ssPaletteNegative <- colorRampPalette(heatmapColours[arrayInd[1]:1])(255)
...
fig1_ss_legend_grob <- fig1_ss_base %>% 
  legend("Correlation") %>% 
  grobify() %>% 
  getGrob("legend")
...
fig1_ss_labels_text_grob <- textGrob(fig1_ss_labels_text,
                                     x = c(0.18, 0.5, 0.845),
                                     y = 0.1,
                                     just = "bottom",
                                     gp = gpar(fontsize = 10,
                                               col = seedColours))
# fig1_ss_labels_title_grob <- textGrob("Human seed regions",
#                                       x = 0.5, y = 0.5,
#                                       gp = gpar(fontsize = 11))
# fig1_ss_labels_grob <- gTree(children = gList(fig1_ss_labels_text_grob,
#                                               fig1_ss_labels_title_grob))
fig1_ss_labels_grob <- fig1_ss_labels_text_grob
...
fig1_linechart_title_grob <- textGrob("Similarity profiles of human seed regions",
                                      x = 0, y = 0.5,
                                      just = "left")
```

```{r results1-fig1-grob, fig.width=10, fig.height=9}
...
#Grob for figure 1
fig1_plots_grob <- arrangeGrob(gTree(children = gList(rectGrob(), textGrob("A.", gp = gpar(fontsize = 14, fontface = "bold")))), #1
                               gTree(children = gList(rectGrob(), fig1_heatmap_title_grob)), #2
                               gTree(children = gList(rectGrob(), textGrob("B.", gp = gpar(fontsize = 14, fontface = "bold")))), #3
                               gTree(children = gList(rectGrob(), fig1_ss_title_grob)), #4
                               gTree(children = gList(rectGrob(), fig1_heatmap_labsmouse_title_grob)), #5
                               gTree(children = gList(rectGrob(), fig1_heatmap_labsmouse_grob)), #6
                               fig1_ss_labels_grob, #7
                               gTree(children = gList(rectGrob(), fig1_heatmap_labshuman_title_grob)), #8
                               gTree(children = gList(rectGrob(), fig1_heatmap_labshuman_grob)), #9
                               fig1_heatmap_main_grob, #10
                               fig1_ss_grob, #11
                               fig1_ss_legend_grob, #12
                               gTree(children = gList(rectGrob(), textGrob("C.", gp = gpar(fontsize = 14, fontface = "bold")))), #13
                               gTree(children = gList(rectGrob(), fig1_linechart_title_grob)), #14
                               fig1_linechart_ytitle_grob, #15
                               fig1_linechart_ytext_grob, #16
                               fig1_linechart_panel_grob, #17
                               gTree(children = gList(rectGrob(), fig1_linechart_legend_grob)), #18
                               ...
                               widths = unit(c(0.1, 0.3, 0.45, 5, 0.5, ssWidth, 0.7, 0.1), 'inch'),
                               # widths = unit(c(0.1, 0.3, 0.45, matWidth, 0.5, ssWidth, 1.73), 'inch'),
                               heights = unit(c(0.1, 0.3, 0.3, 0.4, matHeight, 0.1, 0.2, 1.1, 1.1, 0.2), 'inch'))

fig1_border_grob <- rectGrob(gp = gpar(fill = NA))

sliceAMBAWidth <- 1.1
sliceAMBAHeight <- sliceAMBAWidth/asp
sliceAMBALegend_panel_grob$vp <- viewport(x = unit(9.15, 'inch'),
                                          y = unit(0.8, 'inch'),
...
fileout <- str_c(pathHome, "Draft/", "Version5/", "Figure1.RData")
save(fig1_grob,
     sliceAMBALegend_panel_grob,
     file = fileout)
```
...
meanCb <- matSim_H88M67_AllGenes[indCbHuman, indCbMouse] %>% 
  as.numeric() %>% 
  mean()

sdCb <- matSim_H88M67_AllGenes[indCbHuman, indCbMouse] %>% 
  as.numeric() %>% 
  sd()
...
```{r}
matSim_H88M67_AllGenes %>% 
  t() %>% 
  .[,c("precentral gyrus", "cuneus")] %>% 
  cor()
```

```{r}
matSim_H88M67_AllGenes["crus I", indCbMouse]

meanCrus1 <- matSim_H88M67_AllGenes["crus I", indCbMouse] %>% 
  as.numeric() %>% 
  mean()

sdCrus1 <- matSim_H88M67_AllGenes["crus I", indCbMouse] %>% 
  as.numeric() %>% 
  sd()
