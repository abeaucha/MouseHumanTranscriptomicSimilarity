...
## Panel A: Cortical clustering heatmap

```{r results3-fig5-panelA-heatmap-processing}
...
```{r results3-fig5-panelA-heatmap-base}
heatmapPalette <- magma(n = 100, direction = 1, begin = 0.35) #Not bad
heatmapAnnotationColours_CxType <- c(brewer.pal(n = 9, name = "Blues")[4],
...
```{r results3-fig5-panelA-heatmap-legend}
#1. Legend scale
...
# fig5_heatmap_legend_scale_width <- 0.5  #Width
...
fig5_heatmap_title_grob <- textGrob("Title panel A",
                                    x = 0,
                                    y = 0.5,
                                    just = c("left", "centre"))
```
...
```{r results3-fig5-panelB-wcss-processing}
...
fig5_wcss_title_grob <- textGrob("Title panel B",
                                 x = 0,
                                 y = 0.5,
                                 just = c("left", "centre"))
```
...
```{r results3-fig5-panelC-regionaldist}
...
fig5_regionaldist_title_grob <- textGrob("Title panel C",
                                         x = 0,
                                         y = 0.5,
                                         just = c("left", "centre"))
```
...
fig5_cortexdist_title_grob <- textGrob("Title panel D",
                                       x = 0,
                                       y = 0.5,
                                       just = c("left", "centre"))
```
...
```{r fig5_grob, fig.width=10, fig.height=7.6}
...
fig5_plots_grob <- arrangeGrob(textGrob("A.", gp = gpar(fontsize = 14, fontface = "bold")), #1
                               fig5_heatmap_title_grob, #2
                               textGrob("B.", gp = gpar(fontsize = 14, fontface = "bold")), #3
                               fig5_wcss_title_grob, #4
                               fig5_heatmap_coltree_grob, #5
                               fig5_heatmap_rowtitle_grob,#6
                               fig5_wcss_ytitle_grob, #7
                               fig5_wcss_yaxis_grob, #8
                               fig5_wcss_panel_grob, #9
                               fig5_heatmap_rowtree_grob, #10
                               fig5_heatmap_rowannotations_grob, #11
                               fig5_heatmap_matrix_grob, #12
                               fig5_heatmap_rownames_grob, #13
                               fig5_heatmap_coltitle_grob, #14
                               fig5_heatmap_colnames_grob, #15
                               fig5_heatmap_annotationlegend_grob, #16
                               fig5_wcss_xaxis_grob, #17
                               fig5_wcss_xtitle_grob, #18
                               fig5_wcss_legend_grob, #19
                               fig5_heatmap_legend_grob,  #20
                               textGrob("C.", gp = gpar(fontsize = 14, fontface = "bold")), #21
                               fig5_regionaldist_title_grob, #22
                               textGrob("D.", gp = gpar(fontsize = 14, fontface = "bold")), #23
                               fig5_cortexdist_title_grob, #24
                               fig5_regionaldist_ytitle_grob, #25
                               fig5_regionaldist_yaxis_grob, #26
                               fig5_regionaldist_panel_grob, #27
                               fig5_cortexdist_ytitle_grob, #28
                               fig5_cortexdist_yaxis_grob, #29
                               fig5_cortexdist_panel_grob, #30
                               fig5_regionaldist_xaxis_grob, #31
                               fig5_cortexdist_xaxis_grob, #32
                               fig5_regionaldist_xtitle_grob, #33
                               fig5_cortexdist_xtitle_grob, #34
                               layout_matrix = rbind(c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA),
                                                     c( 1,  1, NA,  2,  2, NA, NA,  3,  4,  4, NA),
                                                     c(NA, NA, NA,  5,  5,  6, NA,  7,  8,  9, NA),
                                                     c(10, 10, 11, 12, 12, 13, NA,  7,  8,  9, NA),
                                                     c(NA, NA, 14, 15, 15, 16, 16, NA, NA, 17, NA),
                                                     c(NA, NA, 14, 15, 15, 16, 16, NA, NA, 18, NA),
                                                     c(NA, NA, 14, 15, 15, 16, 16, NA, NA, 19, NA),
                                                     c(NA, NA, 14, 15, 15, 20, 20, NA, NA, 19, NA),
                                                     c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA),
                                                     c(21, 21, 22, 22, 22, 22, NA, 23, 24, 24, NA),
                                                     c(25, 26, 26, 26, 27, 27, NA, 28, 29, 30, NA),
                                                     c(NA, NA, NA, NA, 31, 31, NA, NA, NA, 32, NA),
                                                     c(NA, NA, NA, NA, 33, 33, NA, NA, NA, 34, NA),
                                                     c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA)),
                               widths = unit(c(0.25, 0.2, 0.2, 0.9, 2.95, 1.1, 0.4, 0.35, 0.3, 2.9, 0.1), 'inch'),
                               heights = unit(c(0.1, 0.4, 0.5, 2, 0.2, 0.2, 0.4, 0.5, 0.15, 0.4, 2, 0.25, 0.25, 0.25), 'inch'))
...
fileout <- str_c(pathHome, "Draft/", "Version5/", "Figure5.RData")
save(fig5_grob,
     fig5_width,
     fig5_height,
     file = fileout)
...
```{r}
dfSimilarityProfiles_Cx_Avg <- matSim_Cx_MLP %>% 
  as_tibble(rownames = "HumanRegion") %>% 
  pivot_longer(cols = -HumanRegion, names_to = "MouseRegion", values_to = "Correlation")

dfClustLabels_Human_MLP_4 <- cutree(hcHuman_MLP, k = 4) %>% 
  enframe(name = "HumanRegion",
          value = "k4")

dfSimilarityProfiles_Cx_Avg %>% 
  inner_join(dfClustLabels_Human_MLP_4, by = "HumanRegion") %>% 
  filter(k4 == 2) %>% 
  filter(MouseRegion == "Primary somatosensory area") %>% 
  pull(Correlation) %>% 
  mean()
...
```{r}
dfSimilarityProfiles_Cx_MaxSim_Compare %>% 
  group_by(CortexType) %>% 
  summarise(avg = mean(SimilarityMean),
            sd2 = 2*sd(SimilarityMean)) %>% 
  ungroup() 
```
