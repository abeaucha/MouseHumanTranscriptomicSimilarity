...
```{r}
matSim_original <- matSim_H88M67_MLP_mean
```


```{r import-latent-space}
#Generate similarity matrix heatmap and convert to ggplot object
fig2_heatmap_ggplot <- pheatmap(matSim_original,
...
```{r}
fig5_heatmap_ggplot <- pheatmap(mat = t(matSim_original), 
...
# Isocortical subtree (version 1)

```{r import-latent-space}
#Path to MLP files
pathMLP <- "../data/MLP_outcomes_isocortex_v1/"
...
#Compute the average matrix
matSim_isocortex_v1 <- rowMeans(arraySimLatentSpace, dims = 2)
rownames(matSim_isocortex_v1) <- listLabelsHumanReordered$Region88_reordered
colnames(matSim_isocortex_v1) <- listLabelsMouseReordered$Region67_reordered
```

```{r}
matSim_isocortex_v1 <- matSim_isocortex_v1[,colnames(matSim_isocortex_v1) %in% labelsCortexMouse]
matSim_isocortex_v1 <- matSim_isocortex_v1[rownames(matSim_isocortex_v1) %in% labelsCortexHuman,]
```

```{r}
#Generate similarity matrix heatmap and convert to ggplot object
fig2_heatmap_ggplot <- pheatmap(matSim_isocortex_v1,
                                color = heatmapPalette,
                                breaks = heatmapBreaks,
                                cluster_cols = F, cluster_rows = F,
                                border_color = "white",
...
```{r}
fig5_heatmap_ggplot <- pheatmap(mat = t(matSim_isocortex_v1), 
                                cluster_cols = T, cluster_rows = T,
                                cutree_rows = 4, cutree_cols = 4,
                                treeheight_col = 30, treeheight_row = 30,
                                color = heatmapPalette_cluster,
...
#Compute the average matrix
matSim_isocortex_v2 <- rowMeans(arraySimLatentSpace, dims = 2)
rownames(matSim_isocortex_v2) <- listLabelsHumanReordered$Region88_reordered
colnames(matSim_isocortex_v2) <- listLabelsMouseReordered$Region67_reordered
```

```{r}
matSim_isocortex_v2 <- matSim_isocortex_v2[,colnames(matSim_isocortex_v2) %in% labelsCortexMouse]
matSim_isocortex_v2 <- matSim_isocortex_v2[rownames(matSim_isocortex_v2) %in% labelsCortexHuman,]
```

```{r}
#Generate similarity matrix heatmap and convert to ggplot object
fig2_heatmap_ggplot <- pheatmap(matSim_isocortex_v2,
                                color = heatmapPalette,
                                breaks = heatmapBreaks,
                                cluster_cols = F, cluster_rows = F,
                                border_color = "white",
...
```{r}
fig5_heatmap_ggplot <- pheatmap(mat = t(matSim_isocortex_v2), 
                                cluster_cols = T, cluster_rows = T,
                                cutree_rows = 4, cutree_cols = 4,
                                treeheight_col = 30, treeheight_row = 30,
                                color = heatmapPalette_cluster,
