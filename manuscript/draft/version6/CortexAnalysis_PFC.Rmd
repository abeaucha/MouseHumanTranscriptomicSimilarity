...
if(runSimMat){
  ...
  save(listSimCortex,
       file = "CortexSimilarityMatrices.RData")
  
  tf <- Sys.time()  
  
  print(tf-ti)
} else {
  load("CortexSimilarityMatrices.RData")
}
```
...
```
```{r fig.height = 3, fig.width = 3}
nMLP <- length(listSimCortex)

temp <- dfSimilarityProfiles %>% 
  filter(Seed %in% structsHuman_PFC) %>% 
  group_by(Data, Seed) %>% 
  mutate(IsMaximal = ifelse(Similarity == max(Similarity), 1, 0)) %>% 
  group_by(Seed, Target) %>% 
  summarise(prop = sum(IsMaximal)/nMLP) %>% 
  ungroup() %>% 
  rename(Human = Seed,
         Mouse = Target)

temp <- temp %>% 
  mutate(Mouse = factor(Mouse, levels = rev(mouseOrder)),
         Human = factor(Human, levels = structsHuman_PFC))

ggplot(temp, aes(x = Human, y = Mouse, fill = prop)) + 
