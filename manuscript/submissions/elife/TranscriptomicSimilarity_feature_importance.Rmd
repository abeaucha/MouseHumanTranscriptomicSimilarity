...
integrated_grads_structs <- split(x = df_integrated_grads_summary,
                                  f = factor(df_integrated_grads_summary$Region,
                                             levels = c('Caudoputamen', 'Primary motor area', 'Infralimbic area')))
...
```{r fig-ggplot}
ngenes <- 5
ggplot_list <- map(.x = integrated_grads_structs,
                   .f = plot_top_genes,
                   n = ngenes)
...
grid_widths <- c(0.25, 2.8, 0.2, 2.8, 0.2, 2.8, 0.7, 0.25)
grid_heights <- c(0.225, 2.25, 0.1, 0.2, 3, 0.225)

#Grob for figure plots
fig_plots_grob <- arrangeGrob(ggplot_list[['Caudoputamen']],
                              ...
                              layout_matrix = rbind(c(NA, NA, NA, NA, NA, NA, NA, NA),
                                                    c(NA,  1, NA,  2, NA,  3, NA, NA),
                                                    c(NA, NA, NA, NA, NA, NA, NA, NA),
                                                    c(NA,  4, NA,  5, NA,  6, NA, NA),
                                                    c(NA,  4, NA,  5, NA,  6,  7, NA),
                                                    c(NA, NA, NA, NA, NA, NA, NA, NA)),
...
#Save as pdf
pdf(file = 'Figure_feature_importance.pdf',
    width = fig_width,
    height = fig_height)
grid.newpage()
grid.draw(fig_grob)
