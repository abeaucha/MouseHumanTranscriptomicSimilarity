...
#Panel A title
fig1_heatmap_title_grob <- textGrob("Transcriptomic similarity of mouse and human gray matter",
                                    x = 0, y = 0.5,
                                    just = "left")
```

```{r results1-fig1-data1}
fig1_data1 <- matSim_H88M67_AllGenes %>% 
  as_tibble(rownames = "HumanRegion")
write_csv(x = fig1_data1,
          file = "Figure1_Data1.csv")
```
...
```{r results1-fig1-quant-1, eval = F}
...
```{r results1-fig1-quant-2, eval = F}
...
```{r results1-fig1-quant-3, eval = F}
...
```{r results1-fig1-quant-4, eval = F}
...
```{r results1-fig1-quant-7, eval = F}
