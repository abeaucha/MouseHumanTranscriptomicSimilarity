...
fig2_heatmap_title_grob <- textGrob("Average latent space gray matter similarity",
                                    x = 0,
                                    just = "left")
```

```{r results2-fig2-data}
fig2_data <- vector(mode = "list", length = length(listSimLatentSpace))
for (i in 1:length(listSimLatentSpace)) {
  fig2_data[[i]] <- listSimLatentSpace[[i]] %>% 
    as_tibble(rownames = "HumanRegion") %>% 
    pivot_longer(cols = -HumanRegion, 
                 names_to = "MouseRegion",
                 values_to = "Correlation") %>% 
    mutate(Data = names(listSimLatentSpace)[i],
           LatentSpaceID = str_extract(Data, "[0-9]+"),
           LatentSpaceID = as.numeric(LatentSpaceID))
}

fig2_data <- reduce(.x = fig2_data, 
                     .f = bind_rows)

fig2_data <- fig2_data %>% 
  mutate(MouseRegion = factor(MouseRegion, levels = listLabelsMouseReordered$Region67_reordered),
         HumanRegion = factor(HumanRegion, levels = listLabelsHumanReordered$Region88_reordered)) %>% 
  arrange(LatentSpaceID, MouseRegion, HumanRegion) %>% 
  select(LatentSpaceID, MouseRegion, HumanRegion, Correlation)

start <- c(1,(floor(nrow(fig2_data)/3)*(1:2)+1))
end <- c(floor(nrow(fig2_data)/3)*(1:2), nrow(fig2_data))
for (i in 1:length(start)) {
  write_csv(x = fig2_data[start[i]:end[i],],
            file = str_c("Figure2_Data", i, ".csv"))
}
```
