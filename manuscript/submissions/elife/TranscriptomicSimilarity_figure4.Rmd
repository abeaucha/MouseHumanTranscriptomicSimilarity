...
dfCanonicalPairs_H88M67_Ranks_MLP <- dfCanonicalPairs_H88M67_Ranks_MLP %>% 
...
df_proportions <- tibble(Seed = unique(dfCanonicalPairs_H88M67_RankDiffs$Seed),
                         theta = 0,
                         theta_error = 0,
                         theta_lower = 0,
                         theta_upper = 0,
...
for(i in 1:nrow(df_proportions)) {
  ...
  df_model <- dfCanonicalPairs_H88M67_RankDiffs %>% 
...
fig4_ss_array <- mincBuildArray(values = df_proportions$p,
...
```{r results2-fig4-data2}
#Order by mouse region
fig4_data2 <- df_proportions %>% 
  rename(MouseRegion = Seed) %>% 
  arrange(desc(MouseRegion))

#Write to file
write_csv(x = fig4_data2,
          file = "Figure4_Data2.csv")
```
...
```{r results2-fig4-quant-1, eval=F}
df_model <- dfCanonicalPairs_H88M67_RankDiffs %>% 
  group_by(Seed) %>% 
...
```{r results2-fig4-quant-2, eval=F}
...
```{r results2-fig4-quant-3, eval=F}
df_proportions %>% 
  arrange(desc(p))
```

```{r results2-fig4-quant-4, eval=F}
...
```
```{r results2-fig4-quant-5, eval = F}
#Number and proportions of regions with non-positive rank differences in more than 90% latent spaces
dfCanonicalPairs_H88M67_RankDiffs_Summary %>% 
  summarise(nregions = sum(PropNonPos >= 0.90),
            fracregions = nregions/n())
```

```{r results2-fig4-quant-6, eval=F}
...
df_proportions$pval <- pbinom(q = df_proportions$p*nMLPsamples, 
                              size = nMLPsamples,
                              prob = pnull,
                              lower.tail = FALSE)
...
```{r results2-fig4-quant-7, eval=F}
df_proportions %>% 
  filter(p >= 0.8) %>%
  arrange(desc(qval))
```
```{r results2-fig4-quant-8, eval=F}
df_proportions %>% 
  filter(p >= 0.9) %>% 
  nrow()
```

```{r results2-fig4-quant-9, eval=F}
df_proportions %>% 
  filter(p == 1) %>% 
  nrow()
```

```{r results2-fig4-quant-10, eval=F}
...
```{r results2-fig4-quant-11, eval=F}
df_rank_summary <- dfCanonicalPairs_H88M67_Ranks %>% 
  filter(Data != 'AllGenes') %>% 
  group_by(Seed) %>% 
  summarise(RankMean = mean(TargetRank),
            RankRange = max(TargetRank) - min(TargetRank),
            RankSd = sd(TargetRank)) %>% 
  ungroup()

dfCanonicalPairs_H88M67_Ranks_MLP %>% 
  filter(Seed %in% c('Primary auditory area', 'Visual areas', 'Paraflocculus'))
```

```{r results2-fig4-quant-12, eval=F}
...
```{r results2-fig4-quant-13, eval = F}
dfSimilarityProfiles %>% 
  inner_join(dfLabelsMouse %>% 
               select(Region11, Region67) %>% 
               distinct(),
             by = c('Seed' = 'Region67')) %>% 
  rename(Seed_Region11 = Region11) %>% 
  inner_join(dfLabelsHuman %>% 
               select(Region16, Region88) %>% 
               distinct(),
             by = c('Target' = 'Region88')) %>% 
  rename(Target_Region16 = Region16) %>% 
  filter(Seed_Region11 == 'Cerebellar cortex') %>% 
  filter(TargetRank %in% 1:10) %>% 
  filter(Data != 'AllGenes') %>% 
  group_by(Seed, Target_Region16) %>% 
