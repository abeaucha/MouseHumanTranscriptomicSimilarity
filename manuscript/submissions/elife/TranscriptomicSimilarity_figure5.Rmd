...
```{r results3-fig5-data1}
fig5_data1 <- dfSimilarityProfiles_Cx_MaxSim %>% 
  mutate(Data = str_replace(Data, "MLP", "LatentSpace"),
         LatentSpaceID = str_extract(Data, "[0-9]+"), 
         LatentSpaceID = as.numeric(LatentSpaceID), 
         CortexType = ifelse(Seed %in% structsMouse_Cx_sensory, "Sensorimotor", "Supramodal")) %>% 
  arrange(LatentSpaceID) %>% 
  select(Data, MouseRegion = Seed, HumanRegion = Target, MaxCorrelation = Similarity, CortexType)

write_csv(x = fig5_data1,
          file = "Figure5_Data1.csv")
```
...
```{r results3-fig5-data2}
fig5_data2 <- dfSimilarityProfiles_Cx %>% 
  filter(Seed %in% structsMouse_Cx,
         Target %in% structsHuman_Cx) %>% 
  mutate(Data = str_replace(Data, "MLP", "LatentSpace")) %>% 
  select(Data, MouseRegion = Seed, HumanRegion = Target, Correlation = Similarity)

write_csv(x = fig5_data2,
          file = "Figure5_Data2.csv")
```
...
```{r results3-fig5-data3}
fig5_data3 <- dfWCSS %>% 
  mutate(data = case_when(data == "MLP" ~ "AverageLatentSpace",
                          data == "AllGenes" ~ "HomologousGenes")) %>% 
  select(Data = data, Species = axis, k, wcss)

write_csv(x = fig5_data3,
          file = "Figure5_Data3.csv")
```
...
model_lm <- summary(lm(SimilarityMean ~ CortexType, data = dfSimilarityProfiles_Cx_MaxSim_Summary))
model_lm
```

```{r}
beta_estimate <- model_lm$coefficients[2, "Estimate"]
beta_upper <- model_lm$coefficients[2,"Estimate"] + 2*model_lm$coefficients[2, "Std. Error"]
beta_lower <- model_lm$coefficients[2,"Estimate"] - 2*model_lm$coefficients[2, "Std. Error"]

str_c(beta_estimate, ", 95% CI [", beta_lower, ",", beta_upper, "]")
```
...
model_lmer <- summary(lmer(SimilarityMean ~ CortexType + (1|Data), 
                           data = df_model))
model_lmer
```

```{r}
beta_estimate <- model_lmer$coefficients[2, "Estimate"]
beta_upper <- model_lmer$coefficients[2,"Estimate"] + 2*model_lmer$coefficients[2, "Std. Error"]
beta_lower <- model_lmer$coefficients[2,"Estimate"] - 2*model_lmer$coefficients[2, "Std. Error"]
