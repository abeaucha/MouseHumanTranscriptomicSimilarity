...
```{r}
df_tmp <- dfSimilarityProfiles_Cx_MaxSim %>% 
...
summary(lmer(Similarity ~ CortexType + (1|Data), data = df_tmp))
```

```{r}
latent_space_ids <- sort(unique(df_tmp$LatentSpace))
...
y <- matrix(data = 0, nrow = length(x), ncol = nrow(df_params))
colnames(y) <- str_c('y', df_params$id)
for(i in 1:nrow(df_params)) {
  beta0 <- df_params[[i, 'beta0']]
  beta <- df_params[[i, 'beta']]
  y[,i] <- beta0 + beta*x
}

df_lines <- as_tibble(y) %>% 
  mutate(x = x) %>% 
  pivot_longer(cols = -x, names_to = 'id', values_to = 'y')

ggplot(df_lines, aes(x = x, y = y, group = id)) + 
  geom_line(alpha = 0.1) + 
...
model_lm <- summary(lm(SimilarityMean ~ CortexType, data = dfSimilarityProfiles_Cx_MaxSim_Compare))
model_lm
```
...
# model_lmer <- 
summary(lmer(SimilarityMean ~ CortexType + (1|Data), data = df_model))
```

```{r}
latent_space_ids <- sort(unique(df_model$LatentSpace))
sample_ids <- sample(x = latent_space_ids, size = 100, replace = FALSE)
...
```{r}
ggplot(df_params, aes(x = factor(nsamples), y = -1*log10(pval))) +
  geom_point() +
  # geom_beeswarm(cex = 1.5) +
...
```{r results3-fig5-intext-1, eval=F}
...
```{r results3-fig5-intext-2, eval=F}
...
```{r results3-fig5-intext-3, eval=F}
